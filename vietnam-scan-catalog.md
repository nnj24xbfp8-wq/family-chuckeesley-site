...
### Reconciliation of the remaining orphans (second pass — all resolved as dups/known)

Ran each remaining orphan against the actual published entries. Result: the low-number stragglers are almost entirely **alternate/duplicate scans of already-published page 1s**, not new pages. Nothing left to publish cleanly from this set.

- **dad18 = duplicate scan of dad1** — page 1 of `charlie-to-terrie-1970-06-22` ("first action… 15 feet ahead through the brush… 20 June I got 12 letters… the 6%/50% deal… about $2000 when…"). The existing entry already uses dad1 and marks page 2+ as "not yet transcribed"; dad18 is NOT that missing page (it's page 1 again). Add to dedup list. (Minor: dad18 reads "6% deal"; the entry transcribes "50% deal" — an uncertain faded reading, left as-is.)
- **dad101 = duplicate scan of dad20** — page 1 of `peggy-to-terrie-1971-02-10` (Peggy McMaster Eesley → Terrie: the Da Nang month-extension, the CO→Congressman Miller letter, the stainless-steel pattern). Entry already uses dad20 and notes the continuation/page 2 is withheld; dad101 is page 1, not the continuation. Add to dedup list.
- **dad11 (mid) + dad12 (close)** — part of the **spring 1971 Hawaii R&R thread**, NOT a 1970 letter. dad12 ("I'm bringing about $400… make reservations… get a room after you get there… keep studying") is **already cited by** `charlie-to-terrie-1970-03-29-ticket-cash` (in its dating rationale), though neither dad11 nor dad12 is attached as a scan anywhere. dad11 adds the parents-disapprove-of-the-unmarried-meeting content (pairs thematically with the "one room & one bed" Feb 15 letter and the April "parents against Hawaii" letters). **Hold** — likely a real letter but entangled with the ticket-cash dating problem below; Chuck to decide whether it earns its own entry once the R&R-thread dates are settled from envelopes.

### ✅ RESOLVED (July 2026) — `charlie-to-terrie-1970-03-29-ticket-cash` was a misdated DUPLICATE; merged into the 1971 canonical entry

On close read (dad9 vs. the transcribed `charlie-to-terrie-1971-03-29-second-rr-booking`) the two entries proved to be the **same letter**: identical date header ("March 29"), identical "get your ticket under (Mrs. Eesley) to save money," identical 12&ndash;19 April window, identical "receipt enclosed," identical "77 days left. Love Charlie." dad9 and dad169 are two scans of the one page. **Fix applied:** dad9 attached to the canonical `1971-03-29-second-rr-booking` entry; the duplicate `1970-03-29-ticket-cash.md` deleted; a redirect added in `astro.config.mjs` (`/docs/letters/charlie-to-terrie-1970-03-29-ticket-cash/` → the 1971 entry). The "$400" in the old entry was a conflation with the separate **dad12** fragment; the true amount is $100 (pdf transcription). Build re-verified clean (566 pages).

The evidence that dated it to 1971, for the record:
- The "book the Hawaii ticket under **Mrs. Eesley** for the military-spouse/dependent fare" line is the same device as **dad264 = 22 March 1971** R&R orders ("You are listed as Mrs. Eesley… reduced fare"). A dependent R&R fare only exists once Charlie is **deployed** — in **March 1970 he was still in stateside Army training**, not able to host a Hawaii R&R.
- Its own cross-links all point to **1971** letters (`1971-03-22`, `1971-03-31-rr-planning`, `1971-05-29`), and there is already a `charlie-to-terrie-1971-03-29-second-rr-booking` — possibly the same day.

If confirmed, this needs: rename to `charlie-to-terrie-1971-03-29-…`, update `postmarkDate`/`dateRange` to 1971, add a redirect in `astro.config.mjs` from the old `/docs/letters/charlie-to-terrie-1970-03-29-ticket-cash/` URL, and check for inbound links. **Left for Chuck** — date corrections from the physical envelopes are his call, and the slug/redirect change is not one to make on an inference.

**Bottom line of the straggler passes:** the 1970–71 corpus is essentially complete. Remaining unattached scans are duplicates (added to dedup lists above) or fragments whose missing pages are not in this scan set — they need the physical originals/envelopes only Chuck has. Recommend Chuck (1) confirm the two "Feb 15" dates and the ticket-cash 1970→1971 date from envelopes, then (2) consider whether dad11/12 warrants its own entry.
